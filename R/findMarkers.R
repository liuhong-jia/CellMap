...
#' @param tcga.data.u TCGA gene expression data. Default: NULL.
...
searchMarkersByCorr <- function(ref.expr, query.genes, scale.data = TRUE, p.cut = 0.1, seed.num = 10, tcga.data.u = NULL, n.workers = 4) {
	...
	marker.lst <- future.apply::future_lapply(query.genes, function(QGs) {
		QGs <- names(QGs) 
