...
#' @example scMap <- CellMapper(st.data, coord.df, ref.expr, ref.anno,factor.size = 0.1,seed.num = 10,pvalue.num = 0.1,knn = 5,verbose = TRUE)

CellMapper <- function(st.data = st.data,
                       coord.df = coord.df,
                       ref.expr = ref.expr,
                       ref.anno = ref.anno,
                       factor.size = 0.1,
                       ...
                       verbose = TRUE)
{
  ...
  species <- readline(prompt = "Please enter the species (mouse/human): ")
  ifelse(species == "mouse", 
       gene.anno <- readRDS(system.file('data', 'mouse.gene.anno.rds', package = 'CellMapper')),
       gene.anno <- readRDS(system.file('data', 'human.gene.anno.rds', package = 'CellMapper')))
  ...
  ungroup() %>% 
  select(-n) %>% 
  tidyr::spread(key = CellType, value = Proportion, fill = 0) %>% as.data.frame
